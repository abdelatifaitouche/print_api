...
from app.schemas.product_schema import ProductRead , ProductUpdate, ProductCreate , ProductLightRead
...
def get_product_by_id(product_id : str , db:Session=Depends(get_db))->ProductLightRead:
    product : ProductRead = product_service.get_by_id(product_id , db)

    return ProductLightRead.model_validate(product.model_dump(exclude={"raw_materials"}))
