...
class ProductService(BaseService[ProductModel , ProductRead , ProductCreate , ProductUpdate]):
    ...
    def create(self , data : ProductCreate , db : Session)->ProductRead:
        ...
        product : ProductModel = self.repo.create(ProductModel(**(data.model_dump(exclude={"raw_materials"}))) , db)
